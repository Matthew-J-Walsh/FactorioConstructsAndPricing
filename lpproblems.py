...
def solve_manual_factory_optimization_problem(construct: ComplexConstruct, u_j: np.ndarray, cost_function: Callable[[CompiledConstruct, np.ndarray], np.ndarray], inverse_priced_indices: np.ndarray, known_technologies: TechnologicalLimitation, 
    ...
    if not extras is None:
        extra_vectors, extra_costs, extra_true_costs, extra_idents = extras
    else:
        extra_vectors, extra_costs, extra_true_costs, extra_idents = np.zeros((u_j.shape[0], 0)), np.zeros(0), np.zeros((u_j.shape[0], 0)), np.zeros(0)

    vectors, costs, true_costs, idents = construct.reduce(cost_function, inverse_priced_indices, dual, known_technologies, 
                                                          extras=(extra_vectors, extra_costs, extra_true_costs, extra_idents))

    manual_costs = true_costs[-1, :]
    prio_primal, prio_dual = BEST_LP_SOLVER(vectors, u_j, manual_costs)
    if prio_primal is None or prio_dual is None:
        raise RuntimeError("Unable to form factory even with slack.")
    prio_cost = np.dot(prio_primal, manual_costs)
    ...
    while True:
        ...
        primal, dual = BEST_LP_SOLVER(np.concatenate((vectors, true_costs[-1, :].reshape(1, -1)), axis=0), 
    ...
    return primal, dual, vectors, costs, true_costs[:-1, :], idents
